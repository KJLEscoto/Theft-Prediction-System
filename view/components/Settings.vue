    ...
    <div class="flex flex-col gap-5 h-auto w-full">
            ...
            <div class="p-5 dark:bg-custom-800 bg-custom-50 space-y-2">
              ...
              <section class="flex justify-between items-center flex-wrap">
                <div class="flex items-center gap-2 text-xl"> 
                  ...
                  <p class="capitalize dark:text-custom-300 font-medium">{{ user.name }}</p>
                </div>
      ...
      <div 
        ...
        <section class="md:flex grid w-full gap-5">
              ...
              <template #label>
                <div class="flex items-center justify-start gap-1">
                  <UIcon 
                    name="i-lucide-phone" 
                    class="text-lg" />
                  <p class="text-base">Add Phone <span class="font-bold text-red-700 dark:text-red-400">(max: 3 only)</span></p>
                </div>
              </template>
                ...
                <div class="relative w-full">
                  ...
                  <UInput 
                    ...
                    :ui="{
                      rounded: 'rounded',
                      color: error ?
                        { red: { outline: 'bg-red-100 dark:bg-red-50 pl-16 text-custom-900 dark:text-custom-900 focus:ring-1 focus:ring-red-400 border border-red-700 focus:border-red-400 active:ring-red-400 active:border-red-400' } } : 
          ...
          <div class="h-auto w-1 bg-custom-200 dark:bg-custom-700 md:block hidden"></div>

          <section class="w-full flex flex-col gap-5">
            <p class="text-sm text-custom-500 dark:text-custom-300">Selected phone numbers will receive an SMS notification.</p>
...
const handleCheckboxChange = async (phone) => {
  try {
    const response = await $fetch(`http://127.0.0.1:8000/api/user/contact/${phone}`, {
      method: 'PUT',
      headers: {
        Authorization: 'Bearer ' + localStorage.getItem('_token'), // Use 'Bearer' prefix for token
      },
...
const remove = async (phone: Phone) => {
  ...
  if (confirm('Remove this phone number?') === true) {
    try {
      const response = await $fetch(`http://127.0.0.1:8000/api/user/contact/${phone.contact_id}`, {
        method: 'delete',
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('_token'), // Use 'Bearer' prefix for token
        },
...
const validate = (state: any): FormError[] => {
  const errors = []
  if (!state.user.contact_number) errors.push({ path: 'phone', message: 'Required' })
  return errors
}
...
async function onSubmit(event: FormSubmitEvent<any>) {
  ...
  try {
    ...
    console.log("Errorrrororororororororororor")
  }
