<template>
  ...
  <div class="flex flex-col p-5 gap-10 w-full h-auto">
      ...
      <div class="flex flex-col gap-5 w-full sm:w-3/4">
        ...
        <div class="lg:flex gap-10 h-auto w-full">
          ...
          <section
            ...
            <span v-else class="font-black text-[150px] w-52 h-52 dark:bg-custom-700 bg-custom-400 flex items-center justify-center rounded-full">{{ initial }}</span>
            ...
            <div class="flex gap-2 h-full items-center justify-center">
              <UKbd 
                ...
                :class="{
                    'dark:border bg-green-600 dark:border-green-700 text-custom-100 dark:text-green-400 cursor-default px-2 py-1 lowercase': userDetails.status === 'active',
                    'dark:border bg-red-600 dark:border-red-700 text-custom-100 dark:text-red-400 cursor-default px-2 py-1 lowercase': userDetails.status=== 'inactive'
                  }"  />
              <UKbd 
          ...
          <section class="lg:w-3/5 w-full flex flex-col gap-5">
            <div class="p-5 dark:bg-custom-800 bg-custom-50 space-y-2">
              ...
              <section class="flex justify-between items-center flex-wrap">
                <div class="flex items-center gap-2 text-xl"> 
                  ...
                  <p class="capitalize font-medium dark:text-custom-300">{{ userDetails.name }}</p>
                </div>

                <div v-if="userDetails.gender === 'female'" class="flex items-center gap-1 rounded-full cursor-default bg-pink-500 dark:bg-pink-500 text-white px-2 py-1"> 
                  <UIcon name="i-game-icons-female" class="h-4 w-4"/>
                  <p class="capitalize text-xs font-medium">{{ userDetails.gender }}</p>
                </div>

                <div v-else-if="userDetails.gender === 'male'" class="flex items-center gap-1 rounded-full cursor-default bg-blue-500 dark:bg-blue-500 text-white px-2 py-1"> 
                  <UIcon name="i-game-icons-male" class="h-4 w-4"/>
                  <p class="capitalize text-xs font-medium">{{ userDetails.gender }}</p>
                </div>

                <div v-else class="text-sm cursor-default">
                  Undefined
                </div>
              </section>
              ...
              <section class="flex items-center gap-2"> 
                ...
                <p class="capitalize text-base dark:text-custom-300">{{ userDetails.phone_number }}</p>
              </section>

              <section class="flex items-center gap-2"> 
                <UIcon name="i-heroicons-envelope-16-solid" class="h-auto w-5"/>
                <p class="text-base dark:text-custom-300">{{ userDetails.email }}</p>
              </section>
            </div>
            ...
            <div class="p-5 dark:bg-custom-800 bg-custom-50 space-y-2">
              ...
              <section class="flex items-center gap-2"> 
                ...
                <p class="text-base dark:text-custom-300">{{ userDetails.username }}</p>
              </section>
              ...
              <section class="flex gap-2 justify-between items-center"> 
                <div class="flex gap-2">
                  ...
                  <p class="text-base dark:text-custom-300">{{ passwordDisplay }}</p> 
            ...
            <div class="p-5 dark:bg-custom-800 bg-custom-50 space-y-2">
              <div class="space-y-2">
                <section class="flex gap-2">
                  ...
                  <p class="dark:text-custom-300">{{ formatDate(userDetails.created_at) }}</p>
                </section>
                <section class="flex gap-2">
                  <h1 class="font-medium">Account updated:</h1>
                  <p class="dark:text-custom-300">{{ formatDate(userDetails.updated_at) }}</p>
                </section>
              </div>
            </div>
        </section>
    ...
    <hr v-if="userDetails.role == 'client'" class="border-custom-300 dark:border-custom-800" />

    <section v-if="userDetails.role == 'client'" class="flex flex-col gap-3 w-full sm:w-3/4 mx-auto">
      <div class="flex items-center gap-1 font-semibold cursor-default w-1/2">
...
const passwordDisplay = computed(() => {
  ...
  return userDetails.password ? userDetails.password.substring(0, 3) + '***' + userDetails.password.slice(-1) : '';
});
...
async function getUser() {
  try {
    ...
    if (response.ok) {
      ...
      Object.assign(userDetails.name = `${data.user.first_name} ${data.user.middle_initial}. ${data.user.last_name}`);
