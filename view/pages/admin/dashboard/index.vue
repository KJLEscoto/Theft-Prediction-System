<template>
  ...
  <div class="h-auto w-full p-5 flex flex-col gap-5">
    ...
    <div class="grid grid-cols-1 gap-5 w-full h-auto dark:text-custom-400">
      ...
      <section class="grid lg:grid-cols-2 grid-cols-1 gap-5 max-w-full h-auto mt-5">
        <div class="overflow-auto flex flex-col gap-5">
          <div class="flex items-center w-full gap-5">
            <h1 class="min-w-max">Active Accounts</h1>
            <hr class="border-custom-300 dark:border-custom-500 mx-auto w-full">
          </div>
          <div class="flex flex-col overflow-x-scroll  bg-custom-100 dark:bg-custom-900 rounded">
            <div class="flex"> 
              <div v-for="user in activeUsers" :key="user.id" class="flex flex-col gap-1 items-center cursor-default p-5">
                <div class="relative">
                  <img draggable="false" class="rounded-full w-14 h-14 border-2 border-custom-400 dark:border-custom-300" :src="user.avatar"/>
                  <span class="w-3 h-3 bg-green-500 absolute rounded-full bottom-0 right-1"></span>
                </div>
                <span class="capitalize text-xs text-center truncate">{{ user.name }}</span>
                <UKbd 
        ...
        <div class="overflow-auto flex flex-col gap-5">
          ...
          <div class="flex flex-col overflow-x-scroll bg-custom-100 dark:bg-custom-900 rounded">
            <div class="flex"> 
              <div v-for="user in inactiveUsers" :key="user.id" class="flex flex-col gap-1 items-center cursor-default p-5">
                <div class="relative">
                  <img class="rounded-full w-14 h-14 border-2 border-custom-400 dark:border-custom-300" :src="user.avatar"/>
                  <span class="w-3 h-3 bg-red-500 absolute rounded-full bottom-0 right-1"></span>
                </div>
                <span class="capitalize text-xs text-center truncate">{{ user.name }}</span>
                <UKbd 
                  class="text-center rounded-full px-2 py-1 bg-custom-700 text-custom-100 dark:text-custom-200 dark:bg-custom-900 dark:border dark:border-custom-500 text-[9px]"
      ...
      <section class="w-full h-auto grid lg:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-5">
        <section v-for="card in cards" class="p-5 rounded bg-custom-100 dark:bg-custom-900 h-auto flex sm:gap-x-7 gap-x-10 items-center cursor-default">
          <div class="rounded-full p-3 bg-custom-600">
            <UIcon class="w-10 h-10 p-1 text-white" :name="card.icon"/>
          </div>
          <div class="flex flex-col gap-2 justify-start text-custom-600 dark:text-white">
...
import { fetchUsers, fetchActiveUsers, fetchInactiveUsers } from '~/assets/js/users';
...
onMounted(async () => {
  ...
  console.log('mao ni mga userss nigga', users)
