<template>
  ...
  <div class="h-auto w-full p-5 flex flex-col gap-5">
    <h1 class="text-lg font-semibold text-custom-800 dark:text-white">List of Avatars</h1>
    ...
    <section class="flex gap-1 items-center">
      <UButton @click="decrementDisplayCount" class="dark:text-white" icon="i-lucide-minus"/>
      <UInput 
        placeholder="0"
        v-model="totalItems" 
        type="text" 
      ...
      />
      <UButton @click="increment" class="dark:text-white" icon="i-lucide-plus"/>
      <span>
        <UButton 
					@click="onSubmit"
          :disabled="isDisabled"
    ...
    <div class="flex flex-wrap gap-5 p-5 bg-white dark:bg-custom-800 rounded justify-center m-auto w-full max-h-[70vh] overflow-y-scroll">
      <span v-if="avatars.length" v-for="(imgSrc, index) in avatars" :key="index">
        <img class="w-24 h-24 border border-red-600 dark:border-red-800" draggable="false" :src="imgSrc" alt="avatar image">
      </span>
      <div v-else class="text-center text-custom-800 dark:text-white">No items.</div>
    </div>
  </div>
</template>
